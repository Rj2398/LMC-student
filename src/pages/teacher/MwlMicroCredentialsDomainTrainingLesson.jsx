...
import { getMwlContent, getmwlLesson, } from "../../redux/slices/teacher/mwlSlice";
import { completeLesson, startLession, } from "../../redux/slices/student/lessionSlice";
...
const MwlMicroCredentialsDomainTrainingLesson = () => {
  ...
  var simulateMatchingQuizAnswered = location.state?.simulateMatchingQuizAnswered;
    ...
    if (trainingName === "Lesson Prep") {
      dispatch(completeLesson({ lesson_id: lesson_id })).then((response) => {
        ...
        if (data) {
          ...
          navigate(data?.next_lesson_id ? `/teacher/mwl-micro-credentials-domain-training-lesson?lesson_id=${data?.next_lesson_id}&id=${id}` : `/teacher/mwl-micro-credentials-domain-training-subject?id=${id}`);
        }
      });
    } else {
      const hasUnwatched = Object.values(videoState).some((val) => val === false);
    ...
    <>
      <div className="baseline-ass-wrp">
        <div className="back-btn mb-3">
          <Link to={`/teacher/mwl-micro-credentials-domain-training-subject?id=${id}`} >
        ...
        <div className="less-wrapper">
          <div className="less-details">
            {mwlContents?.subject && (
              <>
                <h1>
                  {mwlContents?.subject?.Subject}, {mwlContents?.subject?.level} , {mwlContents?.lesson?.title}
                </h1>
                <h1 style={{ color: "#4126A8" }}> {mwlContents?.subject?.Subject} component of PMSC! </h1>
              </>
            )}
                  ...
                  {mwlessondetail?.lessons?.length > 0 ? (
                    mwlessondetail.lessons.map((lesson) => {
                      ...
                      if (lesson.status === "locked") {
                        imageSrc = "/images/subject-detail/sub-lessons/locked-not-started.svg";
                      } else if (lesson.status === "in-progress") {
                        imageSrc = "/images/subject-detail/sub-lessons/in-progress.svg";
                      } else if (lesson.status === "completed") {
                        imageSrc = "/images/subject-detail/sub-lessons/completed.svg";
                      } else {
                        imageSrc = "/images/subject-detail/sub-lessons/locked-not-started.svg";// fallback image 
                      }
            ...
            <div className="delete-pop-wrap">
              <form>
                ...
                <div className="delete-pop-btn">
                  <a href="#" className="active" onClick={handleCloseModal} aria-label="Close">
                    Review
                  </a>
                  <a href="#">Continue</a>
                </div>
