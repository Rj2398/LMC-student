...
const Classdetails = () => {
  ...
  useEffect(() => {
    if (subjectId) {
      dispatch(getClassDetailBySubject({ subject_id: subjectId, school_id: selectedSchool }));
    }
    if (currentLevel) {
  ...
  useEffect(() => {
    if (selectedCourses?.length > 0) {
      dispatch(getClassDetailBySubject({ subject_id: selectedCourses?.[0], school_id: selectedSchool }));
    }
              ...
              {!currentStudents || currentStudents.length === 0 ? (
                ...
                currentStudents.map((student, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <div className={`status ${student?.status == "not_started" && "inactive"} ${["not_completed", "in_progress", "In Progress", "locked"].includes(student?.status) && "review"}`}>{student?.status}</div>
                    </td>
                    <td>
                      <div className="prog">
                        ...
                        <div className="progress">
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor : `${["not_completed", "in_progress", "In Progress", "locked"].includes(student?.status) ? "#f28100" : "#16a34a"}`
                            }}
                            role="progressbar"
