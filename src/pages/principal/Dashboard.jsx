...
//   }, [ selectedLevel]);
  ...
  useEffect(() => {
	...
	console.log({level_id: selectedLevel})
    }
  ...
  return (
    <>
			...
			<div className="progress-grid principal-progress">
              ...
              <h3>{subDashboard?.overAll_lesson_progress?.percentage}%</h3>
              <p className="text-black"> {subDashboard?.overAll_lesson_progress?.completed} of {subDashboard?.overAll_lesson_progress?.total}  lessons completed</p>
            </div>
					</div>
				</div>
			</div>
					...
					<div className="col-lg-12">
						<div className="my-subjects">
							<div className="my-subjects-head">
								<h3><img src="/images/dashboard/book-icon.svg" alt=''/>{allSubjects?.title||"All classes"}</h3>
								{/* <!-- <a href="#">See details <i className="fa-regular fa-arrow-right"></i></Link> --> */}
							</div>

							<div className="my-subjects-grid">
				 {allSubjects?.classes?.map((subject, index) => (
                    ...
                    <Link to={`/principal/class-detail/${subject?.subject_id}`} state={{ subjectId: subject?.subject_id }}>
